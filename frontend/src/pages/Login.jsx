...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // ✅ Première connexion — changement password obligatoire
    ...
    <div style={{ display: 'flex', height: '100vh', width: '100%', fontFamily: "'Inter', sans-serif", overflow: 'hidden' }}>
      <div style={{ width: '38%', height: '100vh', display: 'flex', flexDirection: 'column', padding: '48px 56px', background: 'white' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 'auto' }}>
          ...
          <span onClick={() => navigate('/')} style={{ fontSize: 12, color: '#94a3b8', cursor: 'pointer' }}>← Retour à l'accueil</span>
        </div>
          ...
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', borderBottom: '1px solid #cbd5e1', paddingBottom: 8, gap: 10 }}>
                ...
                <input type="email" placeholder="votre@mitech.tn" value={email} onChange={e => setEmail(e.target.value)} required
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                ...
                <span onClick={handleForgotPassword} style={{ fontSize: 12, color: '#94a3b8', cursor: 'pointer' }}>Mot de passe oublié ?</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', borderBottom: '1px solid #cbd5e1', paddingBottom: 8, gap: 10 }}>
      ...
      <div style={{ width: '62%', height: '100vh', position: 'relative', overflow: 'hidden' }}>
        <img src="/car_interior1.jpg" alt="Mitech" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center' }} />
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.18)' }} />
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '0 56px 52px' }}>
          <h2 style={{ fontSize: 40, fontWeight: 500, color: 'white', margin: '0 0 12px', lineHeight: 1.2 }}>Excellence &amp; Précision</h2>
          <p style={{ fontSize: 16, color: 'rgba(255,255,255,0.7)', fontWeight: 300, maxWidth: 420, margin: 0 }}>Plateforme de gestion intégrée pour les équipes Mitech Tunisie.</p>
        </div>
      </div>
    </div>
  );
