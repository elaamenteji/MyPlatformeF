...
exports.changePassword = async (req, res) => {
  ...
  try {
    ...
    if (!ok)
      return res.status(400).json({ success: false, message: 'Ancien password incorrect.' });
    const hash = await bcrypt.hash(nouveau, 12);
    // ✅ Met must_change_password = false après changement
    await pool.query(
      'UPDATE users SET mot_de_passe=$1, must_change_password=false WHERE id=$2',
      [hash, req.user.id]
    );
